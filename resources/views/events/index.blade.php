    ...
    <x-slot name="header">
        <div class="flex justify-between">
            <h2 class="font-semibold text-xl text-white leading-tight">
                {{ 'Events' }} {{-- Ubah judul menjadi Events --}}
            </h2>
            <a href="{{ route('events.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded-md">ADD</a> {{-- Perbarui tautan untuk menambah event --}}
        </div>
    </x-slot>
                                ...
                                <tr>
                                    ...
                                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">
                                        @if($event->gambar_events)
                                            <a href="{{ Storage::url($event->gambar_events) }}" download>
                                                <img src="{{ Storage::url($event->gambar_events) }}" alt="Gambar Event" width="400" height="400">
                                            </a>
                                        @else
                                            No Image
                                        @endif
                                    ...
                                    <td class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400">
                                        <div class="flex items-center space-x-2">
                                            <a href="{{ route('events.show', $event->id) }}" class="bg-blue-500 hover:bg-blue-600 text-white rounded-full px-4 py-2">
                                                Show
                                            </a>
                                            <a href="{{ route('events.edit', $event->id) }}" class="bg-red-500 hover:bg-yellow-600 text-white rounded-full px-4 py-2">
                                                Edit
                                            </a>
                                            <form method="post" action="{{ route('events.destroy', $event->id) }}">
                                                @csrf
                                                @method('delete')
                                                <button type="submit" class="bg-red-500 hover:bg-red-600 text-white rounded-full px-4 py-2">
                                                    Delete
                                                </button>
                                            </form>
                                        </div>
